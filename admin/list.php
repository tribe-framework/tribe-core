        ...
        <tbody>
            ...
            if ($ids) {
                foreach ($ids as $arr) {
                    ...
                    foreach ($types[$type]['modules'] as $module) {
                        if (
                            isset($module['list_field']) &&
                            $module['list_field'] &&
                            (
                                isset($module['restrict_id_max']) ?
                                $post['id']<=$module['restrict_id_max'] :
                                true
                            ) &&
                            (
                                isset($module['restrict_id_min']) ?
                                $post['id'] >= isset($module['restrict_id_min']) :
                                true
                            )
                        ) {
                            $module_input_slug_lang = $module['input_slug'].(
                                (
                                    isset($module['input_lang']) && is_array($module['input_lang'])
                                ) ?
                                '_'.$module['input_lang'][0]['slug'] :
                                ''
                            );
                            ...
                            if (
                                (isset($module['list_non_empty_only']) && $module['list_non_empty_only']) &&
                                !trim($cont)
                            ) {
                                $donotlist=1;
                            }
                    ...
                    $tr_echo.='<td>'.(($session_user['role']=='admin' || $session_user['user_id']==$dash->get_content_meta($post['id'], 'user_id'))?'<a href="/admin/edit?type='.$post['type'].'&id='.$post['id'].($type=='user'?'&role='.$_GET['role']:'').'"><span class="fas fa-edit"></span></a>&nbsp;':'').'<a target="new" href="/'.$post['type'].'/'.$post['slug'].'"><span class="fas fa-external-link-alt"></span></a></td></tr>';
