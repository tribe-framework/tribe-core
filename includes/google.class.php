...
class google {
	...
	function curl_api ($url, $params=array(), $method='GET', $body_params='', $file_path='', $content_type='application/json') {
		return json_decode(shell_exec("curl -v -X ".$method." '".$url.(empty($params)?'':'?'.http_build_query($params))."' -H 'Content-type: ".$content_type."' -H 'Authorization: Bearer ".$this->access_token."' ".(is_array($body_params)?"-d '".json_encode($body_params)."'":"")." ".(trim($file_path)?"--data-binary '@".$file_path."'":"")), true);
	}
}
?>